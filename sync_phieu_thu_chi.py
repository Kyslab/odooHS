"""
sync_phieu_thu_chi.py
Import Sổ Nhật Ký Chung MISA → Odoo 17 (Bút toán nhật ký)
- Phiếu thu (PT*), Phiếu chi (PC*), Nghiệp vụ khác (NVK*)...
...
Usage:
    py -3 sync_phieu_thu_chi.py <file.xlsx> [--journal <id>]

    --journal <id>  : Journal ID trong Odoo (mặc định 7 = CSH1 Tiền mặt)
                      VD: --journal 3  (MISC - Hoạt động khác)
"""
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: py -3 sync_phieu_thu_chi.py <file.xlsx> [--journal <id>]")
        sys.exit(1)
    ...
    if '--journal' in sys.argv:
        ...
        if idx + 1 < len(sys.argv):
            JOURNAL_ID = int(sys.argv[idx + 1])
            print(f"→ Dùng Journal ID={JOURNAL_ID}\n")
