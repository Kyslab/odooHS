...
class CashReceiptInvoiceLine(models.TransientModel):
    """Một dòng hóa đơn chưa thu trong wizard"""
    _name = 'cash.receipt.invoice.line'
    _description = 'Dòng hóa đơn chưa thu'
    _order = 'invoice_date asc, invoice_name asc'

    wizard_id = fields.Many2one(
        'cash.receipt.invoice.wizard',
        required=True,
        ondelete='cascade',
    )
    ...
    # Char thuần — chỉ hiển thị số CT, không kèm diễn giải như Many2one display_name
    ...
    def _onchange_chiet_khau_amount(self):
        """Nhập chiết khấu → tự giảm tiền mặt thu = số dư còn lại − chiết khấu"""
...
class CashReceiptInvoiceWizard(models.TransientModel):
    """Wizard thu tiền theo hóa đơn — chọn khách hàng → load HĐ chưa thu → thu tiền + đối trừ"""
    _name = 'cash.receipt.invoice.wizard'
    _description = 'Thu Tiền Theo Hóa Đơn'
    ...
    # ── Chọn / bỏ chọn tất cả (onchange — xử lý client-side, không mất data) ──
    ...
    def _load_invoices(self, date_from=False, date_to=False, so_ct=False, tk=False, dien_giai=False):
        ...
        # Tìm TK chiết khấu mặc định (635) và khoản mục CP (731)
    ...
    def action_confirm(self):
        ...
        # Kiểm tra tổng (tiền mặt + chiết khấu) không vượt số dư
        # Đọc từ invoice_tk131_line_id (model thật) để tránh transient field bị reset
        for ln in selected_lines:
            inv_name = ln.invoice_id.name if ln.invoice_id else '?'
            settle = ln.amount_to_pay + ln.chiet_khau_amount
            # Lấy số dư thực từ account.move.line gốc, không dùng transient field
        ...
        dien_giai = self.dien_giai or self.ly_do or 'Thu tiền theo hóa đơn'

        # ── Chuẩn bị reconcile pairs ────────────────────────────────────
        # Luôn đọc invoice_id.name trực tiếp để tránh related field bị mất
        reconcile_pairs = []
        total_cash = 0.0
        for ln in selected_lines:
            inv_name = ln.invoice_id.name if ln.invoice_id else ''
        ...
        # ── Tạo bút toán ────────────────────────────────────────────────
        move_lines = [
            # Nợ TK tiền mặt — chỉ phần thu tiền mặt thực tế
            (0, 0, {
        ...
        for pair in reconcile_pairs:
            # Nợ TK635 — phần chiết khấu (nếu có)
            if pair['chiet_khau_amount'] > 0 and pair['chiet_khau_account_id']:
                ...
                # Thêm khoản mục phân tích nếu có
            ...
            # Có TK131/1388/1283 — toàn bộ số thanh toán (tiền mặt + chiết khấu)
