...
class BankPaymentInvoiceLine(models.TransientModel):
    """Một dòng công nợ phải trả chưa thanh toán trong wizard ngân hàng"""
    _name = 'bank.payment.invoice.line'
    _description = 'Dòng hóa đơn chưa trả (Ngân hàng)'
    _order = 'invoice_date asc, invoice_name asc'

    wizard_id = fields.Many2one(
        'bank.payment.invoice.wizard',
        required=True,
        ondelete='cascade',
    )
...
class BankPaymentInvoiceWizard(models.TransientModel):
    """Wizard trả tiền ngân hàng theo hóa đơn"""
    _name = 'bank.payment.invoice.wizard'
    _description = 'Trả Tiền Ngân Hàng Theo Hóa Đơn'
    ...
    ly_do = fields.Char(
        ...
        default='Trả tiền ngân hàng theo hóa đơn',
    )
    dien_giai = fields.Char(
        string='Diễn giải',
    )
    ...
    def action_confirm(self):
        ...
        dien_giai = self.dien_giai or self.ly_do or 'Trả tiền ngân hàng theo hóa đơn'
        ...
        for ln in selected_lines:
            ...
            reconcile_pairs.append({
                'invoice_cr_line': ln.invoice_tk331_line_id,
